...
describe("Battle2", function () {
    ...
    before(async () => {
        ...
        this.initLP = "10000"
        this.initSpearPrice = "0.45"
        this.initShieldPrice = "0.55"
        this.feeRatio = "0.03"
	...
	let deadline = Math.floor(new Date().getTime()/1000)+1800
        let txBuySpear = await this.battle.buySpear(parseEther("20"), 0, deadline)
        await txBuySpear.wait()
	await getPriceStatus(this.battle, this.cri)
	console.log("=======================")
        const spearWillGet1 = await this.battle.tryBuySpear(parseEther("20"))
	console.log(`spear will get1 ${formatEther(spearWillGet1)}`)
        await this.battle.buySpear(parseEther("375"), 0, deadline)
	await getPriceStatus(this.battle, this.cri)
	console.log("======================")
        const {cDeltaSpear, cDeltaShield, deltaSpear, deltaShield, lpDelta} = await this.battle.tryAddLiquidity(parseEther("210582"))
	console.log(formatEther(cDeltaSpear), formatEther(cDeltaShield), formatEther(deltaSpear), formatEther(deltaShield), lpDelta)
        const tx = await this.battle.addLiquidity(parseEther("210582"), deadline)
	await getPriceStatus(this.battle, this.cri)
    })
