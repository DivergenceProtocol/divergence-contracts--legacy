...
library Pricing {
    ...
    function getVirtualOut(uint cDeltaAmount, uint cAmount, uint vAmount) internal pure returns(uint) {
        ...
        uint cLimitAmount = DMath.sqrt(cAmount*vAmount.mul(99).div(100));
        uint vLimitAmount = DMath.sqrt(cAmount*vAmount.mul(100).div(99));
    ...
    function getCollateralOut(uint vDeltaAmount, uint vAmount, uint cAmount) internal view returns(uint) {
        // console.log("vDeltaAmount %s", vDeltaAmount / 1e18);
        // console.log("vAmount %s", vAmount / 1e18);
        // console.log("cAmount %s", cAmount / 1e18);
        if (cAmount.divideDecimal(vAmount) > 0.99e18) {
            // console.log("%s", 0);
            uint maxAmountBy1 = (cAmount - vAmount * 99 /100) * 100 / 199;
            // console.log("maxAmountBy1", maxAmountBy1 / 1e18);
            if (vDeltaAmount <= maxAmountBy1) {
                return vDeltaAmount;
            } else {
                uint numerator = (cAmount - maxAmountBy1) * (vDeltaAmount - maxAmountBy1);
                uint denominator = (vAmount + maxAmountBy1) * (vDeltaAmount - maxAmountBy1);
