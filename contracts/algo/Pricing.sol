...
library Pricing {
    ...
    function getVirtualOut(
        ...
        if (cDeltaAmount + cAmount > cLimitAmount) {
            uint256 result = vAmount - vLimitAmount + (cDeltaAmount - (cLimitAmount - cAmount));
            return result;
        } else {
    ...
    function getCollateralOut(
        ...
        if (cAmount.divideDecimal(vAmount) > 0.99e18) {
            uint256 maxAmountBy1 = ((cAmount - (vAmount * 99) / 100) * 100) / 199;
            if (vDeltaAmount <= maxAmountBy1) {
                return vDeltaAmount;
            } else {
                uint256 numerator = (cAmount - maxAmountBy1) * (vDeltaAmount - maxAmountBy1);
                uint256 denominator = (vAmount + maxAmountBy1) * (vDeltaAmount - maxAmountBy1);
